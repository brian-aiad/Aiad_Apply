...
for command_name in npm uv curl python3; do
...
(cd "$web_root" && exec npm run dev -- --hostname 127.0.0.1 --port 3000) \
  >"$web_log" 2>&1 &
web_pid=$!
...
(cd "$repository_root" && exec uv run aiadapplyv2 worker --api-url "$local_url") \
  >"$worker_log" 2>&1 &
worker_pid=$!
sleep 1
...
python3 - "$process_file" "$repository_root" "$web_pid" "$worker_pid" <<'PY'
